...
std::string GetSliceIdentifier(const ::cartographer::mapping::SubmapId& submap_id, const int slice_id)
{
...
OgreSlice::OgreSlice(const ::cartographer::mapping::SubmapId& id, int slice_id, Ogre::SceneManager* const scene_manager,
...
void OgreSlice::Update(const ::cartographer::io::SubmapTexture& submap_texture)
{
    ...
    std::vector<char> rgb;
    CHECK_EQ(submap_texture.pixels.intensity.size(), submap_texture.pixels.alpha.size());
    ...
    const std::string texture_name = absl::StrCat(kSubmapTexturePrefix, GetSliceIdentifier(id_, slice_id_));
    texture_ = Ogre::TextureManager::getSingleton().loadRawData(
        texture_name, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, pixel_stream, submap_texture.width,
        submap_texture.height, Ogre::PF_BYTE_RGB, Ogre::TEX_TYPE_2D, 0);
    ...
    Ogre::TextureUnitState* const texture_unit =
        pass->getNumTextureUnitStates() > 0 ? pass->getTextureUnitState(0) : pass->createTextureUnitState();
...
void OgreSlice::SetVisibility(bool visibility)
{
