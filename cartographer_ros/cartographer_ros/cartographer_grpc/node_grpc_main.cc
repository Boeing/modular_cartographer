...
void Run()
{
    ...
    auto map_builder = absl::make_unique<::cartographer::cloud::MapBuilderStub>(FLAGS_server_address, FLAGS_client_id);

    if (!FLAGS_load_state_filename.empty() && !FLAGS_upload_load_state_file)
    {
        map_builder->LoadStateFromFile(FLAGS_load_state_filename, FLAGS_load_frozen_state);
    }
    ...
    {
        node.SerializeState(FLAGS_save_map_filename, false /* include_unfinished_submaps */);
    }
}
