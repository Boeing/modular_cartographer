...
rosbag2_storage::SerializedBagMessage PlayableBag::GetNextMessage(cartographer_ros_msgs::msg::BagfileProgress* progress)
{
    CHECK(IsMessageAvailable());
...
std::tuple<rosbag2_storage::SerializedBagMessage, int, std::string, bool> PlayableBagMultiplexer::GetNextMessage()
{
    ...
    const bool publish_progress =
        ...
        node_->now() - bag_progress_time_map_[current_bag.bag_id()] >= rclcpp::Duration(progress_pub_interval_, 0);
