...
PlayableBag::PlayableBag(const std::string& bag_filename, const int bag_id, const ros::Time start_time,
                         const ros::Time end_time, const ros::Duration buffer_delay,
    ...
    : bag_(absl::make_unique<rosbag::Bag>(bag_filename, rosbag::bagmode::Read)),
      view_(absl::make_unique<rosbag::View>(*bag_, start_time, end_time)), view_iterator_(view_->begin()),
      finished_(false), bag_id_(bag_id), bag_filename_(bag_filename),
      duration_in_seconds_((view_->getEndTime() - view_->getBeginTime()).toSec()), message_counter_(0),
      buffer_delay_(buffer_delay), filtering_early_message_handler_(std::move(filtering_early_message_handler))
{
...
rosbag::MessageInstance PlayableBag::GetNextMessage(cartographer_ros_msgs::BagfileProgress* progress)
{
    CHECK(IsMessageAvailable());
...
bool PlayableBag::IsMessageAvailable() const
{
    return !buffered_messages_.empty() &&
           (buffered_messages_.front().getTime() < buffered_messages_.back().getTime() - buffer_delay_);
}

int PlayableBag::bag_id() const
{
    return bag_id_;
}
...
{
    bag_progress_pub_ = pnh_.advertise<cartographer_ros_msgs::BagfileProgress>("bagfile_progress", 10);
...
std::tuple<rosbag::MessageInstance, int, bool> PlayableBagMultiplexer::GetNextMessage()
{
    CHECK(IsMessageAvailable());
    ...
    const bool publish_progress =
        ...
        ros::Time::now() - bag_progress_time_map_[current_bag.bag_id()] >= ros::Duration(progress_pub_interval_);
    ...
    return std::make_tuple(std::move(msg), current_bag.bag_id(), !current_bag.IsMessageAvailable());
}
