...
{
    submap_query_service_property_ =
        new ::rviz::StringProperty("Submap query service", kDefaultSubmapQueryServiceName,
                                   "Submap query service to connect to.", this, SLOT(Reset()));
    tracking_frame_property_ = new ::rviz::StringProperty("Tracking frame", kDefaultTrackingFrame,
                                                          "Tracking frame, used for fading out submaps.", this);
    slice_high_resolution_enabled_ = new ::rviz::BoolProperty(
        "High Resolution", true, "Display high resolution slices.", this, SLOT(ResolutionToggled()), this);
    slice_low_resolution_enabled_ = new ::rviz::BoolProperty("Low Resolution", false, "Display low resolution slices.",
                                                             this, SLOT(ResolutionToggled()), this);
    client_ = update_nh_.serviceClient<::cartographer_ros_msgs::SubmapQuery>("");
    trajectories_category_ =
        new ::rviz::Property("Submaps", QVariant(), "List of all submaps, organized by trajectories.", this);
    visibility_all_enabled_ =
        new ::rviz::BoolProperty("All", true, "Whether submaps from all trajectories should be displayed or not.",
    ...
    fade_out_start_distance_in_meters_ = new ::rviz::FloatProperty("Fade-out distance", 1.f,
    ...
    Ogre::ResourceGroupManager::getSingleton().addResourceLocation(
        package_path + kMaterialsDirectory + kGlsl120Directory, "FileSystem", ROS_PACKAGE_NAME);
    Ogre::ResourceGroupManager::getSingleton().addResourceLocation(
        package_path + kMaterialsDirectory + kScriptsDirectory, "FileSystem", ROS_PACKAGE_NAME);
...
{
    client_ =
        update_nh_.serviceClient<::cartographer_ros_msgs::SubmapQuery>(submap_query_service_property_->getStdString());
}
...
void SubmapsDisplay::processMessage(const ::cartographer_ros_msgs::SubmapList::ConstPtr& msg)
{
    ...
    for (const ::cartographer_ros_msgs::SubmapEntry& submap_entry : msg->submap)
    {
        ...
        if (it != trajectory_submaps.end() && it->second->version() > submap_entry.submap_version)
        {
    ...
    for (const ::cartographer_ros_msgs::SubmapEntry& submap_entry : msg->submap)
    {
        ...
        if (trajectory_submaps.count(id.submap_index) == 0)
        {
            ...
            trajectory_submaps.emplace(
                id.submap_index,
                absl::make_unique<DrawableSubmap>(id, context_, map_node_, trajectory_visibility.get(),
                                                  trajectory_visibility->getBool(), pose_markers_visibility->getBool(),
                                                  kSubmapPoseAxesLength, kSubmapPoseAxesRadius));
            trajectory_submaps.at(id.submap_index)->SetSliceVisibility(0, slice_high_resolution_enabled_->getBool());
            trajectory_submaps.at(id.submap_index)->SetSliceVisibility(1, slice_low_resolution_enabled_->getBool());
        }
    ...
    for (const auto& trajectory_by_id : trajectories_)
    {
        ...
        for (auto it = trajectory_submaps.begin(); it != trajectory_submaps.end();)
        {
            if (listed_submaps.count(SubmapId{static_cast<int>(trajectory_id), it->first}) == 0)
            {
...
void SubmapsDisplay::update(const float wall_dt, const float ros_dt)
{
    ...
    try
    {
        const ::geometry_msgs::TransformStamped transform_stamped =
            tf_buffer_.lookupTransform(*map_frame_, tracking_frame_property_->getStdString(), kLatest);
        for (auto& trajectory_by_id : trajectories_)
        {
            for (auto& submap_entry : trajectory_by_id.second->submaps)
            {
                submap_entry.second->SetAlpha(transform_stamped.transform.translation.z,
    ...
    if (context_->getFrameManager()->getTransform(*map_frame_, kLatest, position, orientation))
    {
...
void SubmapsDisplay::ResolutionToggled()
{
    ...
    for (auto& trajectory_by_id : trajectories_)
    {
        for (auto& submap_entry : trajectory_by_id.second->submaps)
        {
            submap_entry.second->SetSliceVisibility(0, slice_high_resolution_enabled_->getBool());
            submap_entry.second->SetSliceVisibility(1, slice_low_resolution_enabled_->getBool());
        }
    }
}
...
Trajectory::Trajectory(std::unique_ptr<::rviz::BoolProperty> property, const bool pose_markers_enabled)
    : visibility(std::move(property))
{
    ...
    pose_markers_visibility = absl::make_unique<::rviz::BoolProperty>(
        ...
        QString("Toggles the submap pose markers of this trajectory."), visibility.get());
    ::QObject::connect(pose_markers_visibility.get(), SIGNAL(changed()), this, SLOT(PoseMarkersEnabledToggled()));
}
