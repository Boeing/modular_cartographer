...
std::unique_ptr<carto::io::PointsBatch>
    HandleMessage(const T& message, const std::string& tracking_frame, const std::shared_ptr<tf2_ros::Buffer> tf_buffer,
        ...
        for (size_t trajectory_id = 0; trajectory_id < bag_filenames_.size(); ++trajectory_id)
        {
            ...
            while (bag_reader.has_next())
            {
                ...
                while (!delayed_messages.empty() &&
                       delayed_messages.front().time_stamp < message->time_stamp - kDelay.nanoseconds())
                {
                    const auto delayed_message = delayed_messages.front();
                    ...
                    for (auto topic_info : bag_metadata.topics_with_message_count)
                    {
                        if (topic_info.topic_metadata.name == delayed_message.topic_name)
                        {
                            ...
                            if (topic_info.topic_metadata.type == "sensor_msgs/msg/LaserScan")
                            {
                                ...
                                points_batch = HandleMessage(laser_scan_msg, tracking_frame, tf_buffer,
                            ...
                            else if (topic_info.topic_metadata.type == "sensor_msgs/msg/MultiEchoLaserScan")
                            {
                                ...
                                multi_echo_laser_scan_serializer.deserialize_message(&serialized_msg,
                                                                                     multi_echo_laser_scan_msg.get());
                                points_batch = HandleMessage(multi_echo_laser_scan_msg, tracking_frame, tf_buffer,
                                                             transform_interpolation_buffer);
                            }
                            else if (topic_info.topic_metadata.type == "sensor_msgs/msg/PointCloud2")
                            {
                                ...
                                points_batch = HandleMessage(pcl2_scan_msg, tracking_frame, tf_buffer,
                                                             transform_interpolation_buffer);
                            }
                ...
                LOG_EVERY_N(INFO, 10000) << "Processed " << (message->time_stamp - begin_time.nanoseconds()) / 1e9
