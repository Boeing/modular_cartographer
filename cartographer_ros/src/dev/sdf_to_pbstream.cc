...
DEFINE_string(pbstream, "map.pbstream", "Pbstream destination");
DEFINE_string(map_png, "", "Map PNG destination");
DEFINE_string(map_origin, "0,0", "Map origin in meters 'x,y'");
DEFINE_string(map_size, "64,64", "Map size in meters 'x,y'");
DEFINE_string(world_sdf, "", "World SDF file");
DEFINE_double(map_resolution, 0.02, "Map resolution");
DEFINE_string(submap_location, "", "Cartographer Submap Location"); // multitoken
DEFINE_string(free_space, "", "Free space polygon"); //multitoken
...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        {
            std::vector<std::string> map_size_strs = absl::StrSplit(FLAGS_map_size, ',');
            CHECK(map_size_strs.size() == 2);
            map_size.x() = static_cast<int>(std::stod(map_size_strs[0]) / FLAGS_map_resolution);
            map_size.y() = static_cast<int>(std::stod(map_size_strs[1]) / FLAGS_map_resolution);
        }

        Eigen::Vector2d map_origin = Eigen::Vector2d::Zero();
        {
            std::vector<std::string> map_origin_strs = absl::StrSplit(FLAGS_map_origin, ',');
        ...
        if (!FLAGS_free_space.empty())
        {
            ...
            for (size_t i = 0; i < free_spaces.size(); ++i)
            {
                ...
                std::vector<std::string> strs = absl::StrSplit(free_space, ',');
        ...
        const std::vector<std::string> submap_locations = absl::StrSplit(FLAGS_submap_location, ';');
        ...
        for (size_t i = 0; i < submap_locations.size(); ++i)
        {
            ...
            std::vector<std::string> strs = absl::StrSplit(submap_location, ',');
            CHECK(strs.size() == 4);
            ...
            const int submap_cells_offset_x = static_cast<int>(submap_tl_x / FLAGS_map_resolution);
            const int submap_cells_offset_y = static_cast<int>(submap_tl_y / FLAGS_map_resolution);

            const int submap_cells_x = static_cast<int>(submap_size_x / FLAGS_map_resolution);
            const int submap_cells_y = static_cast<int>(submap_size_y / FLAGS_map_resolution);
            ...
            auto grid = absl::make_unique<cartographer::mapping::ProbabilityGrid>(
                cartographer::mapping::MapLimits(
                    FLAGS_map_resolution, Eigen::Vector2d(submap_max_x + map_origin.x(), submap_max_y + map_origin.y()),
