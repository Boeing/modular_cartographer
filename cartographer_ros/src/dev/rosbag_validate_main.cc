...
const std::set<std::string> kPointDataTypes = {
    std::string(ros::message_traits::DataType<sensor_msgs::PointCloud2>::value()),
    std::string(ros::message_traits::DataType<sensor_msgs::MultiEchoLaserScan>::value()),
    std::string(ros::message_traits::DataType<sensor_msgs::LaserScan>::value())};

std::unique_ptr<std::ofstream> CreateTimingFile(const std::string& frame_id)
{
    auto timing_file = absl::make_unique<std::ofstream>(std::string("timing_") + frame_id + ".csv", std::ios_base::out);

    (*timing_file) << "# Timing information for sensor with frame id: " << frame_id << std::endl
                   << "# Columns are in order" << std::endl
                   << "# - packet index of the packet in the bag, first packet is 1" << std::endl
...
void CheckImuMessage(const sensor_msgs::Imu& imu_message)
{
    ...
    if (std::isnan(linear_acceleration.norm()) || linear_acceleration.norm() < kMinLinearAcceleration ||
        linear_acceleration.norm() > kMaxLinearAcceleration)
    {
        LOG_FIRST_N(WARNING, 3) << "frame_id " << imu_message.header.frame_id << " time "
                                << imu_message.header.stamp.toNSec() << ": IMU linear acceleration is "
                                << linear_acceleration.norm() << " m/s^2,"
                                << " expected is [" << kMinLinearAcceleration << ", " << kMaxLinearAcceleration
                                << "] m/s^2."
...
void CheckOdometryMessage(const nav_msgs::Odometry& message)
{
    ...
    if (!IsValidPose(pose))
    {
        LOG_FIRST_N(ERROR, 3) << "frame_id " << message.header.frame_id << " time " << message.header.stamp.toNSec()
...
void CheckTfMessage(const tf2_msgs::TFMessage& message)
{
    for (const auto& transform : message.transforms)
    {
        ...
        {
            LOG_FIRST_N(ERROR, 1) << "Input contains transform message from frame_id \"" << transform.header.frame_id
                                  << "\" to child_frame_id \"" << transform.child_frame_id
...
{
  public:
    template <typename MessageType> void CheckMessage(const MessageType& message)
    {
        ...
        if (previous_time_to_it != frame_id_to_previous_time_to_.end() && previous_time_to_it->second >= time_from)
        {
            if (previous_time_to_it->second >= time_to)
            {
                LOG_FIRST_N(ERROR, 3) << "Sensor with frame_id \"" << frame_id << "\" is not sequential in time."
                                      << "Previous range message ends at time " << previous_time_to_it->second
                                      << ", current one at time " << time_to;
            }
            else
            {
                LOG_FIRST_N(WARNING, 3) << "Sensor with frame_id \"" << frame_id << "\" measurements overlap in time. "
                                        << "Previous range message, ending at time stamp "
                                        << previous_time_to_it->second << ", must finish before current range message, "
                                        << "which ranges from " << time_from << " to " << time_to;
            }
            double overlap = cartographer::common::ToSeconds(previous_time_to_it->second - time_from);
        ...
        if (it != frame_id_to_range_checksum_.end())
        {
            ...
            if (previous_checksum == current_checksum)
            {
                LOG_FIRST_N(ERROR, 3) << "Sensor with frame_id \"" << frame_id
                                      ...
                                      << "Range data at time " << current_time_stamp << " equals preceding data with "
    ...
    void PrintReport()
    {
        ...
        {
            LOG(WARNING) << "Sensor with frame_id \"" << it.first << "\" range measurements have longest overlap of "
                         << it.second << " s";
        }
    }

  private:
    typedef std::pair<size_t /* num_points */, Eigen::Vector4f /* points_sum */> RangeChecksum;

    template <typename MessageType>
    static void ReadRangeMessage(const MessageType& message, RangeChecksum* range_checksum,
                                 cartographer::common::Time* from, cartographer::common::Time* to)
    {
        ...
        *from = *to + cartographer::common::FromSeconds(point_cloud[0].time);
        ...
        if (point_cloud.size() > 0)
        {
            ...
            {
                LOG_FIRST_N(ERROR, 1) << "Sensor with frame_id \"" << message.header.frame_id
            ...
            {
                LOG_FIRST_N(INFO, 1) << "Sensor with frame_id \"" << message.header.frame_id
...
void Run(const std::string& bag_filename, const bool dump_timing)
{
    ...
    for (const rosbag::MessageInstance& message : view)
    {
        ...
        {
            frame_id_to_properties.emplace(frame_id, FrameProperties{time, message.getTopic(), std::vector<float>(),
        ...
        if (entry.topic != message.getTopic())
        {
            LOG_FIRST_N(ERROR, 3) << "frame_id \"" << frame_id
                                  << "\" is send on multiple topics. It was seen at least on " << entry.topic << " and "
                                  << message.getTopic();
        }
        ...
        if (dump_timing)
        {
            ...
            (*entry.timing_file) << message_index << "\t" << message.getTime().toNSec() << "\t" << time.toNSec()
                                 << std::endl;
        }
        ...
        if (std::abs(duration_serialization_sensor) > kTimeDeltaSerializationSensorWarning)
        {
            ...
            stream << "frame_id \"" << frame_id << "\" on topic " << message.getTopic() << " has serialization time "
                   << message.getTime() << " but sensor time " << time << " differing by "
                   << duration_serialization_sensor << " s.";
    ...
    if (num_imu_messages > 0)
    {
        ...
        if (std::isnan(average_imu_acceleration) || average_imu_acceleration < kMinAverageAcceleration ||
            average_imu_acceleration > kMaxAverageAcceleration)
        {
            LOG(ERROR) << "Average IMU linear acceleration is " << average_imu_acceleration << " m/s^2,"
                       << " expected is [" << kMinAverageAcceleration << ", " << kMaxAverageAcceleration
    ...
    for (const auto& entry_pair : frame_id_to_properties)
    {
        ...
        float max_time_delta =
            *std::max_element(frame_properties.time_deltas.begin(), frame_properties.time_deltas.end());
        if (IsPointDataType(frame_properties.data_type) && max_time_delta > kMaxGapPointsData)
        {
            LOG(ERROR) << "Point data (frame_id: \"" << entry_pair.first << "\") has a large gap, largest is "
                       << max_time_delta << " s, recommended is [0.0005, 0.05] s with no jitter.";
        }
        if (frame_properties.data_type == ros::message_traits::DataType<sensor_msgs::Imu>::value() &&
            max_time_delta > kMaxGapImuData)
        {
            LOG(ERROR) << "IMU data (frame_id: \"" << entry_pair.first << "\") has a large gap, largest is "
                       << max_time_delta << " s, recommended is [0.0005, 0.005] s with no jitter.";
        }
        ...
        LOG(INFO) << "Time delta histogram for consecutive messages on topic \"" << frame_properties.topic
                  << "\" (frame_id: \"" << entry_pair.first << "\"):\n"
