...
{
  public:
    BagPublisher(const std::string& bag_filename) : Node("rosbag_publisher"), bag_filename_(bag_filename)
    {
        ...
        for (const rosbag2_storage::TopicMetadata& topic_info : topics_)
        {
            ...
            {
                topic_to_publisher_[topic_info.name] =
                    this->create_publisher<sensor_msgs::msg::PointCloud2>(topic_info.name, kQueueSize_)
                        ->get_publisher_handle();
            }
            else if (topic_info.type == "sensor_msgs/msg/MultiEchoLaserScan")
            {
                topic_to_publisher_[topic_info.name] =
                    this->create_publisher<sensor_msgs::msg::MultiEchoLaserScan>(topic_info.name, kQueueSize_)
                        ->get_publisher_handle();
            }
            else if (topic_info.type == "sensor_msgs/msg/LaserScan")
            {
                topic_to_publisher_[topic_info.name] =
                    this->create_publisher<sensor_msgs::msg::LaserScan>(topic_info.name, kQueueSize_)
                        ->get_publisher_handle();
            }
            else if (topic_info.type == "sensor_msgs/msg/Imu")
            {
                topic_to_publisher_[topic_info.name] =
                    this->create_publisher<sensor_msgs::msg::Imu>(topic_info.name, kQueueSize_)->get_publisher_handle();
            }
            else if (topic_info.type == "nav_msgs/msg/Odometry")
            {
                topic_to_publisher_[topic_info.name] =
                    this->create_publisher<nav_msgs::msg::Odometry>(topic_info.name, kQueueSize_)
                        ->get_publisher_handle();
            }
            else if (topic_info.type == "tf2_msgs/msg/TFMessage")
            {
                topic_to_publisher_[topic_info.name] =
                    this->create_publisher<tf2_msgs::msg::TFMessage>(topic_info.name, kQueueSize_)
                        ->get_publisher_handle();
            }
        }
            ...
            for (const rosbag2_storage::TopicMetadata& topic_info : topics_)
            {
                ...
                if (topic_info.name == message->topic_name)
                {
                    if (topic_info.type == "sensor_msgs/msg/PointCloud2")
                    {
                        ...
                        publish_result =
                            rcl_publish(topic_to_publisher_.at(message->topic_name).get(), msg.get(), NULL);
                    }
                    else if (topic_info.type == "sensor_msgs/msg/MultiEchoLaserScan")
                    {
                        rclcpp::SerializedMessage serialized_msg(*message->serialized_data);
