...
void RunOfflineNode(const MapBuilderFactory& map_builder_factory)
{
    ...
    while (playable_bag_multiplexer.IsMessageAvailable())
    {
        ...
        const auto bag_topic =
            std::make_pair(bag_index, node.node_handle()->resolveName(msg.getTopic(), false /* resolve */));
    ...
    if (::ros::ok() && !(bag_filenames.empty() && FLAGS_save_state_filename.empty()))
    {
